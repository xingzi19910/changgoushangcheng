...
public class UserServiceImpl implements UserService {
    ...
    public int updateUserPoints(Task task) {
        ...
        //从task中获取相关数据
        Map map = JSON.parseObject(task.getRequestBody());
        ...
        //将任务存入到redis中
        redisTemplate.boundValueOps(task.getId()).set("exist", 10, TimeUnit.MINUTES);
        //修改用户积分
        int i = userMapper.updateUserPoints(username, point);
        if (i <= 0) {
            return i;
        }
        //记录积分日志
        ...
        i = pointLogMapper.insertSelective(pointLog);
        if (i <= 0) {
            return i;
        }
        //删除redis中的记录
